...
class ResNet50(Algorithm):
    ...
    def _build_net(self) -> nn.Module:
        ...
        resnet50.fc = nn.Linear(in_features=resnet50.fc.in_features, out_features=self.num_classes)
    ...
    def make_graph(self) -> Tuple[Digraph, str, str]:
        ...
        with graph.subgraph(name='cluster_layer3') as c:
            ...
            label = '\n'.join(label)
            c.node(name=name, label=label, shape='box', style='filled', fillcolor='#ffffff')
            c.edge('net.layer3.0.conv1', 'net.layer3.5.conv3', style='dashed')

            name = 'net.layer3.5.conv3'
            label = [name]
            for key, param in dict(self.net.layer3[5].conv3.named_parameters()).items():
                label.append(f'{key} {str(tuple(param.shape))}')
            label = '\n'.join(label)
            c.node(name=name, label=label, shape='box', style='filled', fillcolor='#ffffff')
        graph.edge('net.layer3.5.conv3', 'net.layer4.0.conv1')
